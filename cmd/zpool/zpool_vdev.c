			...
			if (is_raidz_mirror(&lastrep, &rep, &raidz, &mirror) ||
				...
				if (fatal)
					vdev_error(gettext(
					    ...
					    (unsigned long long)
					    lastrep.zprl_parity,
					    (unsigned long long)rep.zprl_parity,
					    rep.zprl_type);
				else
					return (NULL);
		...
		if (is_raidz_mirror(current, new, &raidz, &mirror) ||
			...
			if (raidz->zprl_parity != mirror->zprl_children - 1) {
				vdev_error(gettext(
				    ...
				    raidz->zprl_type,
				    mirror->zprl_type,
				    (unsigned long long)raidz->zprl_parity,
				    (unsigned long long)
				    mirror->zprl_children - 1,
				    (unsigned long long)mirror->zprl_children);
				ret = -1;
			}
			...
			vdev_error(gettext(
			    ...
			    (unsigned long long)current->zprl_parity,
			    (unsigned long long)new->zprl_parity);
			ret = -1;
		} else if (current->zprl_children != new->zprl_children) {
			vdev_error(gettext(
			    "mismatched replication level: pool uses %llu-way "
			    "%s and new vdev uses %llu-way %s\n"),
			    (unsigned long long)current->zprl_children,
			    current->zprl_type,
			    (unsigned long long)new->zprl_children,
			    new->zprl_type);
			ret = -1;
		}
	}
