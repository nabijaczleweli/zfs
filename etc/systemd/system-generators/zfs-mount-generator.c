...
line_worker(char *line, const char *cachefile)
{
	...
	if (p_systemd_ignore == NULL) {
		fprintf(stderr, PROGNAME ": %s (%d): not enough tokens!\n",
		    dataset, getpid());
		return (1 << 1);
	}
	...
	if (strcmp(p_encroot, "-") != 0) {
		...
		if (strcmp(dataset, p_encroot) == 0) {
			...
			if (regexec(&uri_regex, p_keyloc,
				...
				if (0)
					// Add known keylocations that do *not*
					// need mounts here! http:// would fall
			...
			if (!keyloadunit_f) {
				fprintf(stderr, PROGNAME ": %s (%d): "
				    ...
				    dataset, getpid(), keyloadunit, destdir,
				    strerror(errno));
				return (1 << 2);
			}
	...
	if (strcmp(p_systemd_ignore, "-") == 0 ||
		...
		fprintf(stderr, PROGNAME ": %s (%d): "
		    ...
		    dataset, getpid(), p_systemd_ignore);
		return (1 << 3);
	}
	...
	if (strcmp(p_canmount, "on") == 0) {
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid canmount=%s\n",
		    dataset, getpid(), p_canmount);
		return (1 << 4);
	}
	...
	if (strcmp(p_mountpoint, "legacy") == 0 ||
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid mountpoint=%s\n",
		    dataset, getpid(), p_mountpoint);
		return (1 << 5);
	}
	...
	if (mountfile == NULL) {
		fprintf(stderr,
		    PROGNAME ": %s (%d): abnormal simplified mountpoint: %s\n",
		    dataset, getpid(), p_mountpoint);
		return (1 << 6);
	}
	...
	//
	// The sum of all strings here is 93,
	// so the max length is less than half that.
	char opts[128] = "";
	...
	if (strcmp(p_atime, "on") == 0) {
		...
		if (strcmp(p_relatime, "on") == 0)
			...
			fprintf(stderr,
			    PROGNAME ": %s (%d): invalid relatime=%s\n",
			    dataset, getpid(), p_relatime);
	} else if (strcmp(p_atime, "off") == 0) {
		strcat(opts, ",noatime");
	} else
		fprintf(stderr, PROGNAME ": %s (%d): invalid atime=%s\n",
		    dataset, getpid(), p_atime);
	...
	if (strcmp(p_devices, "on") == 0)
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid devices=%s\n",
		    dataset, getpid(), p_devices);
	...
	if (strcmp(p_exec, "on") == 0)
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid exec=%s\n",
		    dataset, getpid(), p_exec);
	...
	if (strcmp(p_readonly, "on") == 0)
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid readonly=%s\n",
		    dataset, getpid(), p_readonly);
	...
	if (strcmp(p_setuid, "on") == 0)
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid setuid=%s\n",
		    dataset, getpid(), p_setuid);
	...
	if (strcmp(p_nbmand, "on") == 0)
		...
		fprintf(stderr, PROGNAME ": %s (%d): invalid nbmand=%s\n",
		    dataset, getpid(), p_setuid);
	...
	// 	Units with canmount=onalways have precedence over noauto.
	...
	// 	name is in the array. If it is, we have multiple noauto
	// 	datasets for the same mountpoint. In such cases, we remove the
	// 	file for safety. To avoid further noauto datasets creating
	...
	FILE *keyloadunit_f = fopenat(destdir_fd, mountfile,
	    O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, "w", 0644);
	if (strcmp(p_canmount, "on") == 0)
		sem_post(&noauto_files->noauto_names_sem);
	if (!keyloadunit_f) {
		fprintf(stderr,
		    PROGNAME ": %s (%d): couldn't open %s under %s: %s\n",
		    dataset, getpid(), mountfile, destdir, strerror(errno));
		return (1 << 7);
	}
...
static int
pool_enumerator(zpool_handle_t *pool, void *_)
{
	(void) _;
	int ret = 0;
	...
	if (!name || !tsearch(name, &known_pools, STRCMP)) {
		// This technically leaks if there's two pools
		// with the same name.
...
main(int argc, char **argv)
{
	...
	default:
		fprintf(stderr,
		    PROGNAME ": wrong argument count: %d\n", argc - 1);
		return (1);
	}
	...
	while ((cachent = readdir(fslist_dir)) != NULL) {
		...
		while ((read = getline(&line, &linelen, cachefile)) >= 0) {
			...
			case -1:
				fprintf(stderr,
				    PROGNAME ": couldn't fork for %s: %s\n",
				    line, strerror(errno));
				break;
			case 0: /* child */
